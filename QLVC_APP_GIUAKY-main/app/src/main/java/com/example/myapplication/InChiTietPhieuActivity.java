...
public class InChiTietPhieuActivity extends Activity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        maPVC = intent.getStringExtra("inMaPVC");
        DBhelper = new DBHelper(this, "qlvc.sqlite", null, 1);
        //Tìm  Công trình theo mã CT
        Cursor dt = DBhelper.GetData("select * from PVC where maPVC= '" + maPVC.trim() + "'");
        while(dt.moveToNext()){
            pvc = new PhieuVanChuyen(dt.getString(0),dt.getString(1),dt.getString(2), dt.getString(3));
        }
